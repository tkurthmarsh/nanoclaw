 ...
 * Handles code blocks/inline code first (protecting their content),
 * then applies bold/italic/heading conversions.
 */
function markdownToHtml(text: string): string {
  ...
  // Bold: **text** or *text* (single asterisks used by agent for bold in WA-style)
  text = text.replace(/\*\*(.+?)\*\*/g, '<b>$1</b>');
  text = text.replace(/\*([^*\n]+?)\*/g, '<b>$1</b>');

  // Italic: _text_ (skip if looks like a variable/path with multiple underscores)
    ...
    try {
      ...
      if (html.length <= MAX_LENGTH) {
        ...
        for (let i = 0; i < html.length; i += MAX_LENGTH) {
          await this.bot.api.sendMessage(numericId, html.slice(i, i + MAX_LENGTH), { parse_mode: 'HTML' });
        }
      }
